...
### 使用的資料結構的原因
1. 遞迴呼叫堆疊（Call Stack）
Ackermann 函數以遞迴定義，因此最自然的實作方式即為遞迴函式呼叫。
每次呼叫 A(m, n) 都會建立一個新的呼叫堆疊框架（stack frame），記錄：
當前參數 m、n
返回位置（return address）
呼叫結果的暫存值
使用理由：
遞迴直接對應數學定義，程式結構簡潔。
系統自動管理狀態與返回流程，避免手動控制。
2. 模擬堆疊結構（非遞迴版本）
由於 Ackermann 函數的遞迴深度極大，容易導致堆疊溢位（Stack Overflow）。
因此，若使用非遞迴版本，需以**明確的堆疊資料結構（Stack）**模擬遞迴過程。
使用理由：
控制遞迴展開順序，避免系統自動呼叫造成記憶體耗盡。
有助於理解遞迴邏輯如何在底層以堆疊實作。
### 使用的演算法的原因
根據函數定義直接撰寫遞迴式：
1. 遞迴演算法（Recursive Algorithm）
```cpp
int Ackermann(int m, int n) {
    if (m == 0) return n + 1;
...
清楚對應數學公式。
容易實作與理解遞迴的概念。
缺點是遞迴深度極深，可能造成執行時間過長或堆疊溢位。

2. 非遞迴演算法（Iterative Simulation）
利用自建堆疊（stack）模擬函數呼叫過程，
手動儲存變數狀態、回傳值與返回順序。
原因與特點：
可控制記憶體使用量。
適用於需要防止 stack overflow 的情況。
結構較複雜，但可加深對遞迴運作機制的理解。
