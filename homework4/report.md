...
### (a) 高度分析心得
- 這次高度分析實驗讓我發現，BST 的效能非常依賴資料的「隨機性」。
- 當我用 `random_device` 產生數值時，樹的高度增長非常緩慢，這代表搜尋效率很好。
- 但我也在開發時思考，如果今天資料是從檔案讀取的排序資料，BST 就會變成廢紙一張，這也是為什麼後續會有 AVL 樹或紅黑樹這類自動平衡設計的原因。
- 實際算出的 ratio 接近 2，讓我覺得數學理論在程式中跑出來的感覺很神奇。

### (b) 刪除節點心得
- 實作 `deleteNode` 是這學期到目前為止邏輯最繞的地方。
- 特別是在處理「雙子節點」時，要先去右邊找最小、還要遞補數值、最後還要記得釋放原本的記憶體。
- 一開始我漏掉了把 `findMin` 找到的數值覆蓋回根節點的步驟，導致整棵樹斷掉。
- 這次開發讓我學會如何利用遞迴回傳指標來更新父節點的連結，這比用手動指來指去安全很多。

### 開發與設計總結
- 在開發過程中，我將插入改成 `while` 迴圈版本，而刪除則維持 `遞迴` 版本。這樣的混和設計是為了測試自己對兩種邏輯的掌握度。
- 我認為模組化（如將 `findMin` 獨立出來）不僅讓程式變乾淨，在處理刪除邏輯時也變得很直覺。最後通過多個 $n$ 值的測試，確認了程式在處理萬筆資料時依然穩定，這是我這次作業最大的收穫。
